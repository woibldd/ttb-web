    ...
    methods: {
      nextHandle() {
        if(!this.form.email) {
          ...
          if(utils.isAndroid()) {
            let isLogin = android.isLogin()
            if(isLogin) {
              let checkedEmail = JSON.parse(android.verify(this.form.email))
              console.log(checkedEmail)
              if (checkedEmail.code === 0) {
                let nextChecked = JSON.parse(android.insert(this.form.name, this.form.region, this.form.email, this.form.create_time, this.form.update_time))
            ...
            if (utils.isIos()) {
               service.proxyApply({
                ...
                if (res.code === 0) {
                  service.proxyApplyInsert(this.form, {
                    headers: {
                      token: this.$route.query.token
                    }
                  }).then((res) => {
