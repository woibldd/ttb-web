...
const getUnitTotalValue = (futures, holding, multiplier) => {
...
const getUnitLiqPrice = ({ isBuy, leverages, amount, price, available_balance, totalValue, totalAmount }, product) => {
  const { base_risk, gap_risk, im, mm, take_rate, multiplier, max_leverage } = product
  // 全仓ture 逐仓false
  const isCross = +leverages === 0
  // 委托价值
  ...
  console.log(`初始保证金margin${margin}`)
  // const initalMargin = entrustValue.div(!+leverages ? max_leverage : leverages).times(Big(IM).plus(1))
  // 维持保证金【MM百分比（当前挡位）*委托价值】
  ...
  if (!isCross) {
    ...
    console.log({multiplier, amount, price, margin: margin.toString(), MM: MM.toString(), available})
    const lipPrice = Big(multiplier).times(totalAmount.abs()).div(Big(totalValue).minus(margin).plus(MM).minus(available))
    console.log(`强平价格lipPrice：${lipPrice}`)
    return lipPrice
  }
}
