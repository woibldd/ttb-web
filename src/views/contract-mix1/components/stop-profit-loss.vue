    ...
    <div class="content" v-if="order">
      <div class="holding-info">
        ...
        <div class="r2" flex="main:left">
          ...
          <span class="mr-12"> {{(order.product.MIX || {}).current  | round(order.product.price_scale)}} {{` ${order.currency}`}}</span> 
        </div>
      </div>
...
import {submitOrder} from '@/modules/api/contractMix'
...
export default { 
  ...
  methods: {
    ...
    checkParams() {
      this.errmsg = ''
      const lastPrice = (this.order.product.MIX || {}).current || 0
