...
export default {
  ...
  methods: {
    ...
    configure () {
      ...
      config.interval = local.mixInterval
      ...
      this.widget.onChartReady(function () {
        ...
        widget
          ...
          .subscribe(null, function (interval) {
            local.mixInterval = interval 
            widget.save(function(data) { 
              local.mixWidgetContent = data
            })
          })   
        widget.chart().executeActionById('drawingToolbarAction') 
        
        if (local.mixWidgetContent) {
          local.mixWidgetContent.charts[0].panes[0].sources[0].state.symbol = vm.pair
          local.mixWidgetContent.charts[0].panes[0].sources[0].state.shortName = vm.pair.replace('_', '/')
          local.mixWidgetContent.charts[0].chartProperties.paneProperties.background = config.overrides['paneProperties.background']
          widget.load(local.mixWidgetContent)
        } 
    ...
    handleOption(option, p) { 
      if (option === 'fullscreen') { 
        ...
        if (key === '1m') { // 分时线
          ...
          local.mixLineType = this.widget.chart().chartType() // 记录当前的K线样式
          this.widget.chart().setChartType(2) // K线样式切換到线形图 
        } else {
          this.widget.chart().setResolution(key, null) // 周期切换
          this.widget.chart().setChartType(local.mixLineType) // K线样式切換到线形图 
        }
        local.mixIntervalText = p
      }  
    },
    switchSkin (skin) { 
      // this.init() 
