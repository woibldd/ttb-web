<template>
  <div class="blend-contract-container"> 
    <!-- <v-nav from="mix"/> -->
    <div ref="blend-container" :class="['page-contract', 'table-container', state.skin]"> 
      <!-- 1-1 -->  
      <div style="width: 100%;" flex>
        ...
        <div class="x-col mt-2 mb-2 "  flex-box="0"   v-if="showSymbolList" style="width:302px;"> 
          <div class="r1-c1-r1 mt-2 mb-2" 
            ...
            <symList ref="symList" :pair="state.blend.pair"/>
          </div>
        </div>
        <!-- 币对信息/k线 -->
            ...
            <div class="r1-c2-r1">
              <div class="product-row" flex="left"> 
                <div  class="product-info"> 
                  <div flex="box:first cross:center" style="height: 60px;">
                    <div >
                      <span v-if="showSymbolList"><label class="f17">{{ activeProduct.product }}/{{item.currency}}</label> </span>
                      <el-popover 
                        v-else
                        ...
                        <div class="drop-down">
                          ...
                          <div v-for="(item, idx) in products" :key="idx" class="drop-item">  
                            <div 
                              :class="[{'router-link-exact-active': item.symbol===state.blend.pair}, 'link']"
                              @click="handleProductsChange(item)">
                              <el-row >
                                <el-col :span="10" class="label pl-10"><span class="currency">{{item.product}}</span><span class="product">/{{item.currency}}</span> </el-col>
                                <el-col 
                                  :class="[(item.BLEND || {}).increment_24h > 0?'text-success':'text-danger']"
                                  :span="8">
                                  {{`₮${+(item.BLEND || {}).current}`}}
                                </el-col>
                                <el-col 
                                  :class="[(item.BLEND || {}).increment_24h > 0?'text-success':'text-danger', 'txr', ' pr-10']"
                                  :span="6" >
                                  {{+(item.BLEND || {}).change_24h > 0 ? '+' : ''}}{{(item.BLEND || {}).change_24h || 0 | round(2)}}%
                                </el-col>
                              </el-row> 
                            </div> 
                          </div>
                        </div>
                        <span slot="reference"><label class="f20 bold">{{ activeProduct.product }}/{{activeProduct.currency}} <icon name="arrow-down" /> </label> </span>
                      </el-popover>  
                    </div> 
                  </div>
                </div> 
                ...
                <div class="product-info mr-25"  flex="cross:center" > 
                  <div>
                    <div class="tick-info">
                      <label  :class="[(activeProduct.BLEND || {}).increment_24h > 0?'text-success':'text-danger']">{{(activeProduct.BLEND || {}).current}}</label>
                    </div>
                    <div class="">
                      <label class="f11 c-8a mb-6">≈ {{ state.fiatMoneySymbol }}<fiat-money
                            base="USDT"
                            :value="(activeProduct.BLEND || {}).current"/></label>
                    </div> 
                  </div>
                </div> 
                <div  class="product-info mr-25"  flex="cross:center" v-for="(value,key) in mapDishNewInfo" 
                  :key='key'>
                  <div>
                    ...
                    <div class="tick-value">
                      <span v-if="key==='fee_rate'">
                        ...
                        <router-link
                          ...
                          :to="{name: 'BlendContractFee'}"> {{ $big(symbolInfo.fee_rate || 0).times(100).round(4).toFixed(4) }}% </router-link> 
                      </span>
                      ...
                      <span v-else-if="key==='increment_24h'" > 
                        <span v-if="((activeProduct.BLEND || {})['increment_24h'] || 0) > 0" class="text-success">
                          {{(activeProduct.BLEND || {})['increment_24h']}}
                        </span> 
                        <span v-else class="text-danger">
                          {{(activeProduct.BLEND || {})['increment_24h']}}
                        </span>
                      </span> 
                      <span v-else-if="key==='volume_24h'"> {{(activeProduct.BLEND || {})[key] | pretty }}</span>
                      <span v-else>
                        {{(activeProduct.BLEND || {})[key] }}
                      </span> 
                    </div> 
                  </div>
                </div>  
                ...
                <div class="tv-tools">
                  <el-dropdown class="tool ml-10" @command="handleCyclesCommand" size="mini">
                    <span class="el-dropdown-link pointer">
                      <span style="display: inline-block; width:36px; white-space: nowrap;">{{$t(`tv.cycles.${local.blendIntervalText}.text`)}}</span> <i class="el-icon-arrow-down el-icon--right"></i>
                    </span>
              ...
              <div class=""> 
                <div class="delegate-list"
                    ...
                    <orderBook2
                      ...
                      :last-price="(activeProduct.BLEND||{}).current"
        ...
        <div class="r1-c4 x-col mr-4" flex-box="0" style="width:322px;" > 
          <div class="r1-c4-r1 option " style="height:430px" >
              ...
              <div class="option-box-middle"> 
                ...
                <div class="option-input  mt-15"
                  ...
                  <div v-else-if="i===1 && isProportion"
                    ...
                    <i v-else>
                      <span class="text-success">{{ $big(isProportion).times(costObj.buyAmount || 0).round(0,0) }}</span>/
                      <span class="text-danger">{{$big(isProportion).times(costObj.sellAmount || 0).round(0,0)}}</span>
                    </i>
                  ...
                  <template v-else> 
                    <xnumberInput
                      ...
                      <i slot="suffix" class="el-input__icon pr-10">{{ key==='shippingSpace' ? $t('contract_min_unit') : tradingType }}</i>
                      <el-select slot="append" v-if="key==='triggerPrice'" :popper-class="state.skin" v-model="trigger_type">
                        <el-option
                      ...
                      <el-select class="contract_type_select" 
                ...
                <div class="mt-20 mr-20 txr">
                  <!-- {{$t('contract_value')}}： {{totalValue() || 0}} {{tradingType}} -->
                  {{`${$t('contract_total_werehouse_value')}:${ $big(activeAcountAndPriceArr.shippingSpace || 0).times(activeProduct.multiplier || 0)  || 0} ${activeProduct.product}`}} 
                  <!-- {{activeAcountAndPriceArr}} -->
                </div>
                <div v-if="activeBtnsKey < 3" class="option-proportion" >
                  <!-- <el-button-group style="width: 100%;" flex>
                    <el-button :type="isProportion===0.1 ? 'primary' : ''" @click="handleProportion(0.1)">10%</el-button>
                    <el-button :type="isProportion===0.25 ? 'primary' : ''" @click="handleProportion(0.25)">25%</el-button>
                    <el-button :type="isProportion===0.5 ? 'primary' : ''" @click="handleProportion(0.5)">50%</el-button>
                    <el-button :type="isProportion===0.75 ? 'primary' : ''" @click="handleProportion(0.75)">75%</el-button>
                    <el-button :type="isProportion===1 ? 'primary' : ''" @click="handleProportion(1)">100%</el-button>
                  </el-button-group> --> 
                  <div class="ix-slider" > 
                    <ix-slider 
                      ref="sliderBuy"
                        ...
                        <el-button 
                          ...
                          <div
                            ...
                            <span
                              ...
                              @ {{ activeAcountAndPriceArr['value']||(activeProduct.BLEND||{}).current }}  
                            </span>
                            <span
                            ...
                            <p class="mt-5" flex="main:center cross:center">
                              <b v-if="+activeTypesKey===1">
                                {{ $tR(`mapFormContent.cost`) }}：<span>{{ !activeAcountAndPriceArr['shippingSpace']?'--': $big(costObj[key]).round(2,0) }}</span> 
                                <!-- {{ (activeBalance||{}).currency }} -->
                              </b>
                              <b v-else>
                                <!-- 可平仓数量：{{ costObj[`${key}Amount`] }} 张 -->
                                <!-- {{`可平${key === 'buy'? '空':'多'}：${ costObj[`${key}Amount`] } 张`}} -->
...
export default {
  ...
  computed: {
    ...
    currentPair() { 
      if (this.activeProduct)
        return `${this.activeProduct.product}/${this.activeProduct.currency}`
      else 
        return ''
    },
    formValueObj () { 
      if (!this.state.userInfo || !this.activeProduct.BLEND || !this.costObj || !this.activeBalance || !this.activeProduct.MARKET) return {}
      const price = this.activeAcountAndPriceArr['value'] || this.activeProduct.BLEND.current
      ...
      return {
        // 1: calcMixValueByAmountAndPrice(this.activeAcountAndPriceArr['shippingSpace'], price, this.activeProduct.multiplier, this.activeBalance.rate),
        // 2: this.costObj[this.side === 1 ? 'buy' : 'sell'],
        // 3: this.activeBalance.available_balance,
        // 4: +(balance.holding || 0 ) + (+this.activeTypesKey === 2 ? -this.activeAcountAndPriceArr['shippingSpace'] : +this.activeAcountAndPriceArr['shippingSpace']),
        // 5: this.activeProduct.MARKET.current,
        // 6: getLiqPrice,
        // 7: 1 - (this.activeProduct.BLEND.current - getLiqPrice) / this.activeProduct.BLEND.current
        ...
        difference: 1 - (this.activeProduct.BLEND.current - getLiqPrice) / this.activeProduct.BLEND.current,
        differenceb: this.$big(this.activeProduct.BLEND.current - getLiqPrice).round(2).abs() 
      }
    },
    userData () {
    ...
    currentData () {
      return this.products.reduce((prev, curr) => {
        prev[curr.currency] = (curr.BLEND || {}).current
        return prev
      }, {})
    }, 
    asks () {
    ...
    mapHandlers () {
      return {
        shipping: api.getBlendBalanceList,
        shipped: api.getBlendClosedpositionList,
        curEntrust: api.getBlendActiveOrders,
        lossEntrust: api.getBlendActiveTriggers,
        historyEntrust: api.getBlendOrderHistory,
        bargain: api.getBlendOrderFills
      }
    },
    tableColumns () {
            ...
            switch (key) {
              ...
              case 'price':
                if (+row.type===6 || +row.type===4 || +row.type===2) { 
                  // return this.$t('contract_market_price')
    ...
    calcTableList () {
      const obj = {
        ...
        bargain:  this.tableList,
        historyEntrust: this.tableList,
      }
    ...
    activeProductPrice () {
      if (this.activeProduct && this.activeProduct.BLEND) {
        return this.activeProduct.BLEND.current
      }
      return 0
    },
    mapProduct () {
  ...
  async created () {    
    ...
    this.rates = (await api.getRates()).data
    actions.updateSession()  
    
    const res = await api.getBlendSymbolList() 
    if (res && !res.code) {
      this.products = res.data
      this.state.blend.pairList = this.products
    } 
    const product = this.products.find(item => item.symbol === this.$route.query.pair) || this.products[0]  
    if (!this.$route.query.pair) { 
      const pair = this.products.find(item => item.symbol === local.blend) ? local.blend : this.products[0].symbol 
      this.$router.replace({
        name: 'blend',
        query: {
          pair
      ...
      api.getBlendSymbolInfo({ symbol: this.$route.query.pair }).then(res => {
        if (!res.code) {
    ...
    this.state.blend.pair = product.symbol
    ...
    this.$eh.$on('blend:handleAmount', this.handleAmountObj)
  },
  methods: {
    swithStyle() {
      let ele = this.$refs['blend-container']
    ...
    handleProportion(num) { 
      ...
      if (+this.activeTypesKey === 1) {
        ...
        quantity = amount.div(this.$big(this.activeProduct.multiplier).times(this.activeAcountAndPriceArr['value'] || this.activeProduct.BLEND.current)).times(num).round(0, 0)
    ...
    checkActive () {
      // 先检查是否开通合约
      api.checkContractActive('BTCUSD', this.state.isSimulation)
        .then(res => {
          if (!res.code) {
    ...
    setTitle () {
      if (this.activeProduct) {
        document.title = (this.activeProduct.BLEND ? this.activeProduct.BLEND.current : '...') +
    ...
    totalValue () {
      ...
      return getTotalValue(this.entrustList,
        {
          amount: this.activeAcountAndPriceArr['shippingSpace'],
          price: this.activeAcountAndPriceArr['value'] || this.activeProduct.BLEND.current },
    ...
    //     isRule = this.costObj[`${side}Amount`] >= +(this.activeAcountAndPriceArr['shippingSpace'] || 0)
    //   }
    //   if (+this.activeBtnsKey > 2) { 
    //     const hasValue = this.activeAcountAndPriceArr['shippingSpace'] && this.activeAcountAndPriceArr['triggerPrice'] 
    //     return hasValue && isRule
    //   } 
    //   else {
    //     return this.activeAcountAndPriceArr['shippingSpace'] && isRule 
    //   } 
    // },
    handleDisabledBtn (side) { 
      let isRule = true  
    ...
    async handleClickLeveragePreview(lever) { 
      ...
      const res = await api.setBlendLeveragePreview(params) 
      let tipObj = null
      if (!res.code) {
        tipObj = res.data[0]
    ...
    setLeverage (leverage) {
      api.setBlendLeverage({ name: this.activeProduct.name, leverage }).then(res => {
        if (!res.code) { 
    ...
    handlePopoverClick (side) { 
      ...
      this.activeAcountAndPriceArr['value'] = this.activeAcountAndPriceArr['value'] || (this.activeProduct.BLEND||{}).current
        ...
        if (!res.code) { 
          let blist = res.data.map(item => { 
            ...
            item.funUnrealized = (row) => { 
              if (!this.mapProduct || !curProduct.BLEND || !row.holding || !row.price) {
                return {}
              } 
              ...
              if (+openValue===0) {
                ...
                let unrealized = calcMixProfit(row.holding, row.price, curProduct.BLEND.current, curProduct.multiplier, row.rate) 
                let roe = toBig(unrealized).div(item.value).times(leverage).times(100).toFixed(this.scale)
                let unrealizedM = calcMixProfit(row.holding, row.price, curProduct.MARKET.current, curProduct.multiplier, row.rate)
                let roeM = toBig(unrealizedM).div(item.value).times(leverage).times(100).toFixed(this.scale)
                return { unrealized, roe, unrealizedM, roeM, openValue }
              } else if (row.side === 2) {
                let unrealized = calcMixProfitLoss(row.holding, row.price, curProduct.BLEND.current, curProduct.multiplier, row.rate) 
                let roe = toBig(unrealized).div(item.value).times(leverage).times(100).toFixed(this.scale)
    ...
    handleProductsChange (product) { 
      ...
      if (!this.$route.query.pair !== product.symbol) { 
        this.$route.query.pair = product.symbol
      ...
      // this.setTitle()
      // this.state.blend.pair = `${product.product}_${product.currency}` 
      // this.state.blend.pair = porduct.symbol

    },
    calcIncreaseRate (product) {
      if (!product.BLEND) return 0
      return bigDiv([bigTimes([product.BLEND.increment_24h, 100]), bigMinus([product.BLEND.current, product.BLEND.increment_24h])], Math.max(2, product.price_scale))
    },
    handleDishInfoItem (key) {
      ...
      if (key === 'markPrice') { 
        ...
        const unit = this.$tR(`sheet`)
        return key === 'volume_24h' ? logogramNum(this.activeProduct.BLEND[key]) + unit : bigRound(this.activeProduct.BLEND[key], fixed)
      }
    },
    ...
    handleAmountObj () { 
      ...
      return new Promise(resolve => {
        this._timer = setTimeout(async () => {
          ...
          const res = await api.getBlendAllAmount()  
          ...
          //获取可平仓数量
          const cres = await api.getBlendClosedpositionList() 
          if (!res.code) { 
    ...
    matchFutureItemByKey (key) {
      ...
      return this.tickersData.BLEND.find(item => item.pair === key)
    },
    ...
    submitOrder (side) {
      ...
      return api.submitBlendOrder(data).then(res => {
    ...
    cancelOrder (data) {
      ...
      api.cancelBlendOrder({ user_id, order_id: id, name }).then(res => {
        if (!res.code) {
          this.handleAmountObj()
          setTimeout(() => {
        ...
        if (match) { 
          ...
          if (this.products && this.products.length > 0) {
            ...
            if (found) {  
              ...
              this.tradingType = found.symbol_currency[0].currency
              this.handleProductsChange (found)
            }  
            api.getBlendSymbolInfo({ symbol: pair }).then(res => {
              if (!res.code) {
