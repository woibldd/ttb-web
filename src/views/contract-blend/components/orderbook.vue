      ...
      <div
        ...
        >
          <table class="table table-asks" :class="[state.skin]">
            <!-- <tbody v-if="state.blend.pairInfo"> -->
            <tbody>
              <tr
                is="OrderbookItem"
...
export default {
  ...
  computed: { 
    ...
    curGroup () {
      if (this.state.blend.pairInfo) {
        const scale = this.state.blend.pairInfo.price_scale
    ...
    indexPriceTips () {
      if (state.blend.pairInfo) {
        return this.$t('contract_index_price_tips', {product_name: state.blend.pairInfo.product_name || 'BTC'})
      }
  ...
  methods: {
    ...
    assignData (data) {
      ...
      // if (bid && !bid.eq(this.state.blend.bid)) {
      //   // this.state.blend.bid = bid
      // }

      // let bidVol = _.get(this.buy, '0.1', 0)
      // if (bidVol && !bidVol.eq(this.state.blend.bidVol)) {
      //   // this.state.blend.bidVol = bidVol
      // }

      // let ask = _.get(this.sell, '0.0', 0)
      // if (ask && !ask.eq(this.state.blend.ask)) {
      //   // this.state.blend.ask = ask
      // }

      // let askVol = _.get(this.sell, '0.1', 0)
      // if (askVol && !askVol.eq(this.state.blend.askVol)) {
      //   // this.state.blend.askVol = askVol
      // }
    ...
    jumpToHistory () {
      this.$router.push({name: 'TradeIndex', params: {pair: this.state.blend.pair}})
    },
    handleScroll (e) {
      this.$nextTick(() => {
        let tag = e.target
