              ...
              <el-popover
                ...
                <div>
                  ...
                  <div
                    ...
                    <p>当前已分配的保证金: <span class="text-primary">{{ item.margin_position | round(item.symbol.price_scale || 2) }}</span> {{ item.currency }} 【最大可减少 <span class="text-primary">{{ item.margin_user | round(item.symbol.price_scale || 2) }}</span> {{ item.currency }}】<span
                      v-if="checked"
                      class="text-primary hover-point"
                      @click="margin_position = Math.floor(item.margin_position)">全部</span></p>
                    <p>可用保证金: <span class="text-primary">{{ item.available_balance | round(item.symbol.price_scale || 2) }}</span> {{ item.currency }}【最大可增加 <span class="text-primary">{{ item.available_balance | round(item.symbol.price_scale || 2) }}</span> {{ item.currency }}】<span
                      v-if="!checked"
                      class="text-primary hover-point"
            ...
            <p
              ...
              <span v-else-if="key==='markPrice'">
                <span v-if="item.product">
                  {{ (item.product.MARKET || {}).current | round(item.symbol.price_scale || 2) }}
                </span>
              </span>
              <span v-else-if="key==='price'">
                <span v-if="item.product">
                  {{ item[key] | round(item.symbol.price_scale || 2) }}
                </span>
              </span>
              ...
              <span v-else>  
                {{ key === 'markPrice'? (item.product.MARKET || {}).current :key === 'liq_price'? bigRound(item[key], item.symbol.price_scale || 2):item[key] }}
              </span>
            </p>
            <p
              v-else
              ...
              <span
                ...
                <span class="text-primary"> 
                  {{ item[key] | round(item.symbol.price_scale || 2) }} {{ item.currency }}
                </span>
                <br>
                <!-- <span>
                  ≈ {{ translateByRate(item[key])|round(2) }} USDT
                </span> -->
              </span> 
              <span
                class="unrealized"
                ...
                <span
                  ...
                  <br>
                  <!-- <span>
                    ≈ {{ translateByRate(item['funUnrealized'](item)['unrealized'])|round(2) }} USDT
                  </span> -->
                </span>
                <span
                  class="marketPrice">
                  ...
                  <br>
                  <!-- <span>
                    ≈ {{ translateByRate(item['funUnrealized'](item)['unrealizedM'])|round(2) }} USDT
                  </span> -->
                </span>
              </span>
              <span
              ...
              <span v-else-if="key==='value'">
                <span>
                    {{ item['funUnrealized'](item)['openValue'] |round(item.symbol.price_scale || 2) }} 
                </span>
                <br>
                <!-- <span>
...
export default {
  ...
  methods: {
    ...
    setTransferMargin (item) {
      ...
      api.setBlendTransferMargin({ name: item.name, amount, side: item.side }).then(res => {
        this.$emit('change')
    ...
    async closeStorehouse (item, isMarket) {
      ...
      if (isMarket) {
        ...
        api.closeBlendOrder(mparams).then(res => { 
          // this.$emit('change')
          if (!res.code) {
            this.$eh.$emit('blend:handleAmount')
            this.$message.success(this.$t('tj_cg'))
          } else {
    ...
    cancelOrder (item) {
      ...
      this.loadingHouse = true
      api.cancelBlendOrder({ user_id, order_id: future_close_id, name }).then(res => {
        // this.$emit('change')
        // this.$message.success(this.$t('tj_cg'))
        this.loadingHouse = false
        if (!res.code) {
    ...
    submitOrder (holdingObj) {
      ...
      this.formArrData.forEach((item, index) => {
        ...
        if (item.type) {
          const fun = item.isEdit ? api.modifyBlendOrder : api.submitBlendOrder
  ...
  async created () {
    this.currencyRates = await api.getRates()
  }
}
</script>
