    ...
    <div class="fund-assets-content">
      ...
      <div v-if="tab==='personal'" class="fund-assets-total mt-30 mb-30">
        ...
        <div class="total-value mt-5">
          <!-- <span class="valuation">{{total | round(8)}} BTC</span>  
          <i class="fail">
                  ...
                  <template slot-scope="scope">
                    <div :style="{textAlign:item.headerAlign||'left'}"> 
                      ...
                      <div v-else> 
                        ...
                        <p class="small">
                          ≈ <fiatMoney :base="scope.row.currency" :value="scope.row[item.prop]" :scale="2"/> 
                          {{ state.fiatMoney }} 
                        </p>
                      </div> 
                    </div>
...
export default {
  ...
  computed: {
    ...
    total() {
      return (this.accountList.blend || {}).btcValue 
    },
    accountTotal() {
      let sum = this.$big(0) 
        ...
        return {
          ...
          handleValue: (value, key, row) => {
            if (key === 'create_time') {
              ...
              value =`<span class="large">${row.name.replace(row.currency, '')}</span><span class="small">/${row.currency}</span>`
            } 
            else if (key==='origin') {
  ...
  methods: {
    ...
    //可用余额
    fetchBalanceList() {
      service.getBlendBalanceList().then(res => {
        if (!res.code) {
    ...
    fetchOrderHistory(pageConfig) { 
      ...
      service.getBlendOrderHistory(params).then(res => {
        if (!res.code) {
    ...
    fetchTradeHistory(pageConfig) {
      ...
      service.getBlendTradeHistory(params).then(res => {
        if (!res.code) {
    ...
    fetchFundHistory(pageConfig) {
      ...
      service.getBlendFundHistory(params).then(res => {
        if (!res.code) {
    ...
    fetchRealized(pageConfig) {
      ...
      service.getBlendCloseRealized(params).then(res => {
        if (!res.code) {
    ...
    fetchSymbolList() {
      service.getBlendContractSymList().then(res => {
        if (!res.code) {
