...
import {toBig} from '@/utils/handleNum'
export default {
  data () {
    return {
      state: 'show',
  ...
  methods: {
    ...
    async confirm () { 
      ...
      if (this.type === 'price') {
        ...
        let content = this.$t('contract.edit_cell.edit_price_pop_content', { price: this.newValue })
        let ok = await utils.confirm(this, {
          customClass: 'ix-message-box-wrapper',
          confirmBtnText: this.$t('contract.edit_cell.edit_price_pop_confirm'),
          confirmButtonClass: 'btn--confirm',
          cancelButtonClass: 'btn--cancel',
          title: this.$t('contract.edit_cell.edit_price_pop_title'),
          content
        })
        if (!ok) {
          return
