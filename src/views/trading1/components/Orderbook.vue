<template>
  <div class="ix-panel">
    <div class="ix-header pl-12" flex="box:first" :skin="state.skin">
      <div  class=" mr-20">
    ...
    <div
      ...
      <div
        ...
        <table class="table table-asks" :skin="state.skin">
      ...
      <div
        ...
        <div class="left-part">  
          <div class="last-price">
            <icon name="arrow-up" :class="{'up':delta > 0, 'down':  delta < 0}"/>
            {{ lastPrice }}
          </div> 
        </div>
        <span
  ...
  methods: {
    ...
    async sub () {
      ...
      utils.$tvSocket.$on('message', (data) => {
        if (data.topic && data.topic.indexOf('orderbook') > -1) {
...
.order-split {
  ...
  top: 0;
  bottom:0;
  background: $--ix-order-split-bg;
  z-index: 100;
  &[skin~='dark'] {
    background-color: $--ix-order-split-bg2; 
  }
