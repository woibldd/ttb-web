<template>
  <div class="orderbook-container" v-loading="loading">
    <div :class="['ix-pannel', 'flex', state.skin] " > 
      <div
        class="ix-panel-thead" >
        <div class="table table-ix-orderbook">
          <div class="thead">
            <!-- <div class="th ibt left" /> -->
            <div class="th ibt left pl-15">{{ $t('price') }}</div>
      ...
      <div
        ...
        <div
          ...
          :class="[{displayFlex:hasBtnReturnDish}, 'flex-lr', state.skin]"
        >
        <!-- {{state.ct.lastSide}} --> 
          <span
            class="delta"
...
export default {
  ...
  computed: {
    ...
    bookHeight () {
      // console.log(this.panelHeight,this.navHeight,this.theadHeight)
...
.order-split {
  color: $--contract-orderbook-price;
  padding: 0 12px;
  line-height: 34px;
  font-size: 16px; 
  position: sticky;
