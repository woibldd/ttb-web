    ...
    <div class="pair-info flex mt-4" style="position:relative">
      ...
      >
        ...
        <!-- <div  class="guide-link" v-show="link">
          ...
          <img @click="link = false" src="@/assets/quxiao.png" width="12" style="    position: relative;left: -4px;top: -20px;cursor: pointer;">
        </div> -->
    ...
    volumnValue () {
      if (this.tick && this.tick.volume_24h) { 
        if (this.pairInfo.currency === 'BTCUSD') {
          ...
          return this.$big(this.tick.volume_24h).times(this.tick.current).times(this.pairInfo.multiplier || 0).round(this.pairInfo.currency_scale || 2).toString()
        }
      }
      return '--'
    },
...
.contract-pair-table {
  color: $--contract-pair-tab;
  font-size: 14px;
  .pair-tab {
    background-color: $--contract-pair-bg;
    .pair-tab-item {
      ...
      &.active {
        background-color: $--contract-pair-active-bg;
      }
    }
  }
  .pair-info {
