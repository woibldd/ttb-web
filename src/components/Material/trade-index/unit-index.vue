...
export default {
    ...
    '$route.params.pair': {
      async handler (pair = '', last) {
        ...
        if (match) {
          ...
          if (!res.code) {
            let prodect = res.data.find(item => item.name.replace('USD', '') === this.coin)
  ...
  computed: {
    symbol () {
      return this.$t('UNIT_&', {currency: this.pair.replace('UNIT_', '').replace('USD', '')})
    },
    coin () {
      return this.pair.replace('USD', '').replace('UNIT_', '')
    },
    tutorialUrl () {
  ...
  methods: {
    _formatPair (type = 'index') {
      return this.pair.replace('UNIT', type.toUpperCase())
    },
    switchChart (type) {
      this.chartType = type
