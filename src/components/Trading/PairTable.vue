<template>
  <div
    ...
    <ul
      ...
      <li
        class="tb"
        v-if="pair.tick">{{ pair.tick.current | fixed(pair.price_scale) }} <span> ≈ {{ state.fiatMoneySymbol }} <fiat-money
          :base="pair.currency"
      ...
      <li
        class="td"
        v-if="pair.tick">{{ pair.tick.lowest_24h | fixed(pair.price_scale) }}</li>
      <li
        class="te"
        v-show="pair.tick">{{ pair.tick.highest_24h | fixed(pair.price_scale) }}</li>
      <li
        class="tf"
        v-if="pair.tick">{{ pretty(pair.tick.volume_24h) }}<span> {{ pair.product }}</span></li>
      <li
        class="tg"
